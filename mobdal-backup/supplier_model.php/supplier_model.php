...
class Supplier_model extends CI_Model {
	...
	function insertSupplierUserData($postData) {
		...
		$dataArray=json_decode($dataToInsert, true);
		//print_r($dataArray);
		//$defect=$dataArray['Defect']!=NULL?$dataArray['Defect']:'0';
	//	$category=$dataArray['Category']!=NULL?$dataArray['Category']:'0';
		//$repair_user_cause_dept=$dataArray['Cause Dept']!=NULL?$dataArray['Cause Dept']:'0';
		//$status=$dataArray['Status']!=NULL?$dataArray['Status']:'0';
		$aoi_detection_status=$dataArray['AOI Detection Status']!=''?$dataArray['AOI Detection Status']:'0';
		$aoi_detection_possibility=$dataArray['AOI Detection Possibility']!=''?$dataArray['AOI Detection Possibility']:'0';
		$dft_detection_status=$dataArray['DFT Detection Status']!=''?$dataArray['DFT Detection Status']:'0';
		$dft_detection_possibility=$dataArray['DFT Detection Possibility']!=''?$dataArray['DFT Detection Possibility']:'0';
		
		//$getDefect=$this->getData('defect',$defect);
		//$getCategory=$this->getData('category',$category);
		//$getCauseDept=$this->getData('cause_dept',$repair_user_cause_dept);
		//$getStatus=$this->getData('status',$status);
		$getAOIDetectStatus=$this->getData('aoi_detection_status',$aoi_detection_status);
		$getAOIDetectPossibillity=$this->getData('aoi_detection_possibility',$aoi_detection_possibility);
		$getDFTDetectStatus=$this->getData('dft_detection_status',$dft_detection_status);
		$getDFTDetectPossibillity=$this->getData('dft_detection_possibility',$dft_detection_possibility);
		//print_r($getAOIDetectStatus);die();
		//print_r($dataArray);die();
		$insertData=array(
			/*"organization"=>array_key_exists('Organization Name', $dataArray)?$dataArray['Organization Name']:'',
			"production_line"=>array_key_exists('Production Line', $dataArray)?$dataArray['Production Line']:'',
			...
			"supplier_cause"=>array_key_exists('Cause', $dataArray)?$dataArray['Cause']:'',
			"aoi_detection_status"=>$getAOIDetectStatus[0]['aoi_detection_status_name'],
			"aoi_detection_possibility"=>$getAOIDetectPossibillity[0]['aoi_detection_possibility_name'],
			"aoi_revision"=>array_key_exists('AOI Revision', $dataArray)?$dataArray['AOI Revision']:'',
			"dft_detection_status"=>$getDFTDetectStatus[0]['dft_detection_status_name'],
			"dft_detection_possibility"=>$getDFTDetectPossibillity[0]['dft_detection_possibility_name'],
			"dft_revision"=>array_key_exists('DFT Revision', $dataArray)?$dataArray['DFT Revision']:'',
			//"cm_report"=>array_key_exists('CM Report', $dataArray)?$dataArray['CM Report']:'',
			//"user_id"=>$this->session->userdata('id'),
			//"user_type"=>$this->session->userdata('user_type')
	...
	function insertLGRepairUserData($postData) {
		...
		$dataToInsert=$postData['updatingClient'];
		$dataArray=json_decode($dataToInsert, true);
		/*$defect=array_key_exists('Defect', $dataArray)?$dataArray['Defect']:'0';
		$category=array_key_exists('Category', $dataArray)?$dataArray['Category']:'0';
		$repair_user_cause_dept=array_key_exists('Cause Dept', $dataArray)?$dataArray['Cause Dept']:'0';
		$status=array_key_exists('Status', $dataArray)?$dataArray['Status']:'0';*/
		$defect=$dataArray['Defect']!=NULL?$dataArray['Defect']:'0';
		$category=$dataArray['Category']!=NULL?$dataArray['Category']:'0';
		$repair_user_cause_dept=$dataArray['Cause Dept']!=NULL?$dataArray['Cause Dept']:'0';
		$status=$dataArray['Status']!=NULL?$dataArray['Status']:'0';
		...
		//	"repair_contents"=>array_key_exists('Repair Contents', $dataArray)?$dataArray['Repair Contents']:'',
			...
			"defect"=>$getDefect[0]['defect_name'],
			"category"=>$getCategory[0]['category_name'],
			"repair_user_cause_dept"=>$getCauseDept[0]['cause_dept_name'],
			"status"=>$getStatus[0]['status_name'],
			//"user_id"=>$this->session->userdata('id'),
			//"user_type"=>$this->session->userdata('user_type'));
	...
	function getData($type,$val='0'){
		//echo $val;
		$result=array();
		$this->db->select("*");
		$this->db->from("master_".$type);
		$this->db->where($type.'_id',$val);
		$query = $this->db->get();	
		if($query->num_rows() > 0){
...
//        return $this->db->query($sql)->result_array();
	...
	function get_selected_all_supplier_data($val=NULL){
		//print_r($this->session->userdata);die('');
		$all_Data=array();
		$i=0;
		$uid=$this->session->userdata('id');
...
//        return $this->db->query($sql)->result_array();
	...
	function get_selected_lg_user_data(){
		$sql = "SELECT *
        FROM `Damaged_pcb_detail` WHERE  `repair_user_cause_dept` !=  'Vendor'";
       // echo $sql;die();
        return $this->db->query($sql)->result_array();
	...
	function delete_supplier_report($serialno,$uid,$reportName){
		$result=array();
		$this->db->select("cm_report");
		$this->db->from('supplier_data');
		$this->db->where('lg_serial_no',$serialno);
		$this->db->where('supplier_id',$uid);
		$query = $this->db->get();
		if($query->num_rows() > 0){
			$result = $query->result_array();
			$cm_report=$result[0]['cm_report'];
			$cm_report_arr=explode(",",$cm_report);
			if (in_array($reportName, $cm_report_arr)){
				$cm_report_arr_flpd=array_flip($cm_report_arr);
				unset($cm_report_arr_flpd[$reportName]);
				$cm_report_arr_reflpd=array_flip($cm_report_arr_flpd);
				$cm_reportData=implode(",",$cm_report_arr_reflpd);
				$insertData=array(
					"lg_serial_no"=>$serialno,
					"supplier_id"=>$uid,
					"cm_report"=>$cm_reportData	
				);
